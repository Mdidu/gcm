...
public class MainUI extends JFrame {
	...
	String[] colonnes= {"Nom","Pr�nom","Adresse","N� SS","Age","Ville"};
	...
	String ville;
	String adresse;
	static PatientDaoJDBC patientDaoJDBC;
	...
	private JTextArea textAreaAdresseEdit;
...
//				String pwd=txtpwd.getText();
				String user="test";
				String pwd="test";
				patientDaoJDBC = new PatientDaoJDBC();
				if(patientDaoJDBC.findUserByUsernameAndPassword(user, pwd) == true) {
					...
					JOptionPane.showMessageDialog(frame, "Veuillez r�essayer une autre fois ","Erreur",JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	...
	public MainUI() {
		...
		JMenuItem mntmNewMenuItem_2 = new JMenuItem("Consulter la liste des PatientDaoJDBCs");
		mntmNewMenuItem_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < listePatient.size(); i++) {
					Patient p=listePatient.get(i);// r�cup�ration de patient num�ro i
				...
				contentPane.updateUI();// mettre � jour le contentPane �pr�s la modification
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(MainUI.this, "Patient est bien ajout�", "Succ�s", JOptionPane.INFORMATION_MESSAGE);
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(row == -1) {
					JOptionPane.showMessageDialog(null, "Attention", "Veuillez sélectionner un patient", JOptionPane.WARNING_MESSAGE);
				}
		...
		panelConsultation.add(btnDelete);
        ...
        lblAdresse.setBounds(218, 148, 46, 13);
        panelModification.add(lblAdresse);
        
        textAreaAdresseEdit = new JTextArea();
        textAreaAdresseEdit.setBounds(335, 145, 264, 86);
        panelModification.add(textAreaAdresseEdit);
        
        JLabel Ville = new JLabel("Ville");
        Ville.setBounds(218, 306, 46, 13);
        panelModification.add(Ville);
        
        textFieldVilleEdit = new JTextField();
        textFieldVilleEdit.setBounds(335, 302, 264, 19);
        ...
        btnValidate.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		...
        		ville = textFieldVilleEdit.getText();
        		adresse = textAreaAdresseEdit.getText();
        		patientDaoJDBC.update(nss, ville, adresse);
        	}
        });
        btnValidate.setBounds(356, 417, 89, 23);
        panelModification.add(btnValidate);
        
        JButton btnCancel = new JButton("Annuler");
        btnCancel.setBounds(483, 416, 89, 23);
        panelModification.add(btnCancel);
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(row == -1) {
	        		...
	        		textAreaAdresseEdit.setText(adresse);
	        		textFieldVilleEdit.setText(ville);
	        		
					JDialog jdialogUpdate = new JDialog(MainUI.this, "Modification du patient");
					jdialogUpdate.getContentPane().add(panelModification);
					jdialogUpdate.setBounds(200, 200, 800, 800);
					jdialogUpdate.setVisible(true);
				}
			}
		});
		panelConsultation.add(btnUpdate);
		
	}
	public void resetFields() {
		textAreaAdresseEdit.setText("");
